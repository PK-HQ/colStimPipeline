function [columnarBitmaps,columnarPCAs]=coregisterBitmap2GreenImgV2(dataStructReference,dataStructSession,bitmapRef,VERpca,plotOrNot)
thresholdVal=0;
...
[imgReferenceCoreg,coregStats,transformParams]=coregisterGreenImages(imgReference,imgTarget,method,filenameStructSession,Mask);
...
bitmapRef90 =bitmapRef(:,:,7);%floor(size(bitmapRef,3)/2) + 1);

%transform the ref VSD column map
bitmapRef0transformed =  columnarBitmaps(:,:,1);
bitmapRef90transformed = columnarBitmaps(:,:,7);%floor(size(bitmapRef,3)/2) + 1);

%0
figure('name','Apply coregistration to bitmap')
subplot(2,3,1)
...
[~,h]=suplabel('Co-registration of reference session to current session','t',[.08 .08 .84 .89]);
set(h,'FontSize',16)
export_fig(pdfFilename,'-pdf','-nocrop','-append');
