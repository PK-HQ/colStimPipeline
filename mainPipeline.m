...
for chamberID=1:2
    ...
    switch analysisMode
        ...
        case {'psypower'}
            %% Power x behavior from saved psycluster output
            finalFile = fullfile( ...
                mainPath, ...
                monkeyName, ...
                'Meta', ...
            ...
            if exist(finalFile, 'file') ~= 2
                error('psypower:MissingPsyclusterFinal', ...
                    ['Required psycluster final file does not exist:\n%s\n\n' ...
                     'Run analysisMode = ''psycluster'' first.'], ...
                    finalFile);
            end

            loadedData = load(finalFile, ...
                'bitmapData', ...
                'analysisBlockID', ...
                'datastruct', ...
                'dataTag', ...
            ...
            mdlStruct = loadedData.mdlStruct;

            if ~strcmp(dataTag, chamberWanted)
                error('psypower:WrongChamber', ...
                    'Loaded final file dataTag=%s, but chamberWanted=%s.', ...
                    dataTag, chamberWanted);
            end

            aggregateModelType = 'weibullfreeAll';
            sourceMdlField = [chamberWanted, aggregateModelType, 'C1'];
            if ~isfield(mdlStruct, sourceMdlField)
                mdlFieldNames = fieldnames(mdlStruct);
                modelFieldPrefix = [chamberWanted, aggregateModelType, 'C'];
                candidateMask = startsWith(mdlFieldNames, modelFieldPrefix) & ...
                    ~endsWith(mdlFieldNames, 'Avg');
                candidateFields = mdlFieldNames(candidateMask);
                hasPowerBehaviorFields = false(size(candidateFields));
                for candidateIdx = 1:numel(candidateFields)
                    candidateMdl = mdlStruct.(candidateFields{candidateIdx});
                    hasPowerBehaviorFields(candidateIdx) = ...
                        isstruct(candidateMdl) && ...
                        isfield(candidateMdl, 'clusterBlocksIdx') && ...
                        isfield(candidateMdl, 'deltaBias');
                end
                candidateFields = candidateFields(hasPowerBehaviorFields);
                if isempty(candidateFields)
                    modelStructFile = fullfile(mainPath, monkeyName, ...
                        'Meta', 'psychometrics', [chamberWanted '-chamber'], ...
                        aggregateModelType, ...
                        ['mdlStruct' chamberWanted '20.mat']);
                    if exist(modelStructFile, 'file') ~= 2
                        error('psypower:MissingModelField', ...
                            ['Saved mdlStruct is missing required field %s, ' ...
                             'and model-specific fallback file does not exist:\n%s'], ...
                            sourceMdlField, modelStructFile);
                    end
                    modelSpecificData = load(modelStructFile, 'mdlStruct');
                    mdlStruct = modelSpecificData.mdlStruct;
                    mdlFieldNames = fieldnames(mdlStruct);
                    candidateMask = startsWith(mdlFieldNames, modelFieldPrefix) & ...
                        ~endsWith(mdlFieldNames, 'Avg');
                    candidateFields = mdlFieldNames(candidateMask);
                    ...
                    candidateFields = candidateFields(hasPowerBehaviorFields);
                    fprintf('Loaded model-specific psycluster file: %s\n', ...
                        modelStructFile);
                end
                if numel(candidateFields) ~= 1
                    error('psypower:MissingModelField', ...
                        ['Could not identify exactly one matching %s* field ' ...
                         'with clusterBlocksIdx and deltaBias. Candidates: %s'], ...
                        modelFieldPrefix, strjoin(candidateFields(:)', ', '));
                end
                sourceMdlField = candidateFields{1};
                fprintf('Using saved psycluster model field: %s\n', ...
                    sourceMdlField);
            end

            clusterMdl = mdlStruct.(sourceMdlField);
            if ~isfield(clusterMdl, 'clusterBlocksIdx') || ...
                    ~isfield(clusterMdl, 'deltaBias') || ...
                    ~isfield(clusterMdl, 'deltaMask')
                error('psypower:MissingPowerBehaviorFields', ...
                    ['Saved model field %s must contain clusterBlocksIdx ' ...
                     'deltaBias, and deltaMask.'], sourceMdlField);
            end

            nBlocks = numel(analysisBlockID);
            clusterBlocks = clusterMdl.clusterBlocksIdx(:);
            if any(clusterBlocks < 1 | clusterBlocks > nBlocks)
                error('psypower:InvalidClusterBlockIndex', ...
                    'Saved clusterBlocksIdx contains indices outside analysisBlockID.');
            end
            ...
            if numel(sessionPower) ~= numel(sessionBias) || ...
                    numel(sessionPower) ~= numel(sessionMask)
                error('psypower:PowerBiasLengthMismatch', ...
                    ['Power values (%d) must align with saved deltaBias ' ...
                     'values (%d) and deltaMask values (%d).'], ...
                    numel(sessionPower), numel(sessionBias), ...
                    numel(sessionMask));
            end

            [~, controlByBlock] = classifyOptostimColumnTargets( ...
                bitmapData.orts, nBlocks);
            controlSessionMask = controlByBlock(clusterBlocks);
            powerBehaviorFigure = plotPowerBehaviorCurves( ...
                sessionPower, sessionBias, sessionMask, controlSessionMask);

            if saveFlag
                reportOutputDir = fullfile('Y:\users\PK\colStimPipeline', ...
                    'outputs');
                if ~exist(reportOutputDir, 'dir')
                    mkdir(reportOutputDir);
                end
                animalID = animalIDForPsychometricReport(monkeyName);
                powerBiasBaseFilename = fullfile(reportOutputDir, ...
                    sprintf('M%d-powerXbehavior-%s-%s', ...
                    animalID, chamberWanted, aggregateModelType));
                print(powerBehaviorFigure, [powerBiasBaseFilename '.svg'], ...
                    '-dsvg', '-painters');
                print(powerBehaviorFigure, [powerBiasBaseFilename '.pdf'], ...
                    '-dpdf', '-painters');
                fprintf('Saved power x behavior figure:\n  %s.svg\n  %s.pdf\n', ...
                    powerBiasBaseFilename, powerBiasBaseFilename);
            end
            ...
            if plotFlag
                ...
                if isfield(mdlStruct, sourceMdlField)
                    ...
                    relevanceStruct = analyzePowerClusterRelevance( ...
                        ...
                        relevanceOpts);
                    expectedRelevanceFigureNames = {'parameterShapley', ...
                        'parameterClusterSummary', ...
                        'parameterBiasByClusterShapley', ...
                        'parameterMaskByClusterShapley', ...
                        'stimulationGroupedShapley', ...
                        'parameterBiasingScatterDiagnostic', ...
                        'parameterMaskingScatterDiagnostic'};
                    nExpectedRelevanceFigures = ...
                        numel(expectedRelevanceFigureNames);
                    nCreatedRelevanceFigures = 0;
                    stagedRelevanceFigureCount = 0;
                    if isfield(relevanceStruct, 'figureHandles')
                        nCreatedRelevanceFigures = ...
                            numel(relevanceStruct.figureHandles);
                    end
                    fprintf('Created relevance figures: %d\n', ...
                        nCreatedRelevanceFigures);
                    if saveFlag
                        ...
                        if saveFlag && plotFlag && ...
                                exist('reportState', 'var') && ...
                                ~isempty(reportState) && ...
                                isfield(relevanceStruct, 'figureHandles') && ...
                                ~isempty(relevanceStruct.figureHandles)
                            relevanceFigureHandles = ...
                                relevanceStruct.figureHandles;
                            if nCreatedRelevanceFigures ~= ...
                                    nExpectedRelevanceFigures
                                warning( ...
                                    'mainPipeline:UnexpectedRelevanceFigureCount', ...
                                    ['Created relevance figures: %d, ' ...
                                    'expected %d. Final psycluster PDF ' ...
                                    'may be missing relevance pages.'], ...
                                    nCreatedRelevanceFigures, ...
                                    nExpectedRelevanceFigures);
                            end
                            nRelevanceFiguresToStage = min( ...
                                nCreatedRelevanceFigures, ...
                                nExpectedRelevanceFigures);
                            for relevanceFigureIdx = ...
                                    1:nRelevanceFiguresToStage
                                relevanceFigureHandle = ...
                                    relevanceFigureHandles( ...
                                    relevanceFigureIdx);
                                relevanceFigureName = ...
                                    expectedRelevanceFigureNames{ ...
                                    relevanceFigureIdx};
                                if ~isgraphics(relevanceFigureHandle, ...
                                        'figure')
                                    warning( ...
                                        'mainPipeline:InvalidRelevanceFigureHandle', ...
                                        ['Relevance figure %d/%d (%s) ' ...
                                        'is not a live figure handle.'], ...
                                        relevanceFigureIdx, ...
                                        nExpectedRelevanceFigures, ...
                                        relevanceFigureName);
                                    continue
                                ...
                                pageCountBefore = ...
                                    getReportPageCount(reportState);
                                reportStateBeforeStage = reportState;
                                try
                                    reportState = stageAndCloseReportFigure( ...
                                        relevanceFigureHandle, reportState);
                                catch stageError
                                    if strcmp(relevanceFigureName, ...
                                            'parameterMaskingScatterDiagnostic')
                                        warning( ...
                                            'mainPipeline:RelevanceFigureStageFallback', ...
                                            ['Normal staging failed for %s: %s. ' ...
                                            'Attempting direct temporary PDF fallback.'], ...
                                            relevanceFigureName, ...
                                            stageError.message);
                                        reportState = ...
                                            fallbackStageAndCloseReportFigure( ...
                                            relevanceFigureHandle, ...
                                            reportStateBeforeStage, ...
                                            relevanceFigureName);
                                    else
                                        rethrow(stageError)
                                    end
                                end
                                [latestRelevancePageFile, ...
                                    latestRelevancePageBytes] = ...
                                    assertReportPageAdded(reportState, ...
                                    pageCountBefore, relevanceFigureName);
                                stagedRelevanceFigureCount = ...
                                    stagedRelevanceFigureCount + 1;
                                fprintf(['Staged relevance figure %d/%d: ' ...
                                    '%s | temp page exists | bytes=%d\n'], ...
                                    relevanceFigureIdx, ...
                                    nExpectedRelevanceFigures, ...
                                    relevanceFigureName, ...
                                    latestRelevancePageBytes);
                            end
                            if stagedRelevanceFigureCount < ...
                                    nExpectedRelevanceFigures
                                error( ...
                                    'mainPipeline:MissingRelevanceFigurePages', ...
                                    ['Staged %d/%d relevance figures into ' ...
                                    'final psychometric PDF.'], ...
                                    stagedRelevanceFigureCount, ...
                                    nExpectedRelevanceFigures);
                            end
                        else
                            fprintf(['No relevance figures staged into final ' ...
                                'psychometric PDF.\n']);
                        end
                        ...
                        fprintf(['Expected relevance pages = %d | ' ...
                            'staged relevance pages = %d\n'], ...
                            nExpectedRelevanceFigures, ...
                            stagedRelevanceFigureCount);
                        fprintf('Final output PDF path: %s\n', finalPdfPath);
                        fprintf('Final PDF byte size: %d\n', pdfInfo.bytes);
                    end
...
function pageCount = getReportPageCount(reportState)
    if ~isfield(reportState, 'pageFiles') || isempty(reportState.pageFiles)
        pageCount = 0;
    else
        pageCount = numel(reportState.pageFiles);
    end
end

function [latestPageFile, latestPageBytes] = assertReportPageAdded( ...
    reportState, pageCountBefore, figureName)
    pageCountAfter = getReportPageCount(reportState);
    if pageCountAfter ~= pageCountBefore + 1
        error('mainPipeline:RelevancePageNotStaged', ...
            ['Staging %s did not add exactly one PDF page. ' ...
            'Before=%d | After=%d.'], ...
            figureName, pageCountBefore, pageCountAfter);
    end

    latestPageFile = getLatestReportPageFile(reportState);
    latestPageBytes = assertReportPageFileExists( ...
        latestPageFile, figureName);
end

function latestPageFile = getLatestReportPageFile(reportState)
    pageCount = getReportPageCount(reportState);
    if pageCount < 1
        error('mainPipeline:MissingReportPageFile', ...
            'Report state does not contain any staged page files.');
    end
    latestPageFile = reportState.pageFiles{pageCount};
end

function pageBytes = assertReportPageFileExists(pageFile, figureName)
    if exist(pageFile, 'file') ~= 2
        error('mainPipeline:MissingReportPageFile', ...
            'Staged PDF page for %s does not exist: %s', ...
            figureName, pageFile);
    end

    pageInfo = dir(pageFile);
    if isempty(pageInfo) || pageInfo.bytes <= 0
        error('mainPipeline:EmptyReportPageFile', ...
            'Staged PDF page for %s is empty: %s', ...
            figureName, pageFile);
    end
    pageBytes = pageInfo.bytes;
end

function reportState = fallbackStageAndCloseReportFigure( ...
    figHandle, reportState, figureName)
    if isempty(figHandle) || ~isgraphics(figHandle, 'figure')
        error('mainPipeline:InvalidFallbackFigureHandle', ...
            'Cannot fallback-stage %s because the handle is not a figure.', ...
            figureName);
    end
    if ~isfield(reportState, 'tempDir') || ...
            exist(reportState.tempDir, 'dir') ~= 7
        error('mainPipeline:MissingReportTempDir', ...
            'Report temporary directory is missing.');
    end

    pageIndex = getReportPageCount(reportState) + 1;
    reportState.pageIdx = pageIndex;
    pageFile = fullfile(reportState.tempDir, ...
        sprintf('page_%04d.pdf', pageIndex));
    drawnow;
    print(figHandle, pageFile, '-dpdf', '-bestfit');
    assertReportPageFileExists(pageFile, figureName);
    reportState.pageFiles{pageIndex} = pageFile;
    close(figHandle);
end
