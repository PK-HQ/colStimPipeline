function [visualr,visualrRatios,PCAr,PCArRatios,optoIntensity]=assessOptoEffect3(dataStructReference,dataStructCurrent,columnarBitmapCoregistered,columnarPCAsCoregistered,saveFlag)
%% Get filenames
...
if isfile(filenameStructCurrent.Intg) % Load imaging data if it exists 
    ...
    % Coregister
    [~,~,transformParams]=coregisterGreenImages(imgReference,imgTarget,method,filenameStructCurrent,Mask);
    % set ROI mask
    ROImask=double(Mask);
    ...
    figure('name','Subtracted columnar responses')
    nCols=4;
    nRows=1;
    [hAx,~]=tight_subplot(nRows,nCols,[.02 .02]);
    colormap(fireice)
   ...
   %}
    ...
    %%
    %{
    figure('name','Intensity x contrast')
    
    % Strength of opto 90 vs visual 90
    for i=1:length(V90O90)
        img=columnarResponse(:,:,V90O90(i)-nblank).*ROImask;
        imgRef=columnarResponse(:,:,V90(i)-nblank).*ROImask;
        [~,~,~,intensityPrct]=calculateSimilarity(img,imgRef);
        O90V90xVisuali(i)=intensityPrct;
    end
    
    % Strength of opto 0 vs visual 0
    for i=1:length(V0O0)
        img=columnarResponse(:,:,V0O0(1)-nblank).*ROImask;
        imgRef=columnarResponse(:,:,V0(i)-nblank).*ROImask;
        [~,~,~,intensityPrct]=calculateSimilarity(img,imgRef);
        O0V0xVisuali(i)=intensityPrct;
    end

    
    % Plot incongruent: visual 0 and 90, with incongruent opto
    plot(stimCon,O90V0xVisuali,'b:','LineWidth',2,'Marker','^','MarkerSize',10,'MarkerFaceColor','r','MarkerEdgeColor','k','DisplayName','Opto-90 + Vis-0 (vs. Vis-0)'); hold on;
    plot(stimCon,O0V90xVisuali,'r:','LineWidth',2,'Marker','v','MarkerSize',10,'MarkerFaceColor','b','MarkerEdgeColor','k','DisplayName','Opto-0 + Vis-90  (vs. Vis-90)'); hold on;
    
    % Plot congruent: visual 0 and 90, with congruent opto
    plot(stimCon,O0V0xVisuali,'b--','LineWidth',2,'Marker','v','MarkerSize',10,'MarkerFaceColor','b','MarkerEdgeColor','k','DisplayName','Opto-0 + Vis-0 (vs. Vis-0)'); hold on;
    plot(stimCon,O90V90xVisuali,'r--','LineWidth',2,'Marker','^','MarkerSize',10,'MarkerFaceColor','r','MarkerEdgeColor','k','DisplayName','Opto-90 + Vis-90  (vs. Vis-90)'); hold on;

    % title and labels
    title({'Matched opto vs visual response', sprintf('Single gabor (S.F.: 6cpd, Con.: %.0f%%)',max(stimCon))},'FontWeight', 'Normal')
    xlabel('Absolute gabor contrast (%)');xlim([min(stimCon) max(stimCon)])
    ylabel('Intensity (r)');
    hLeg=legend('location','southeast','NumColumns',2,'FontSize',10);
    ...
    % Opto 0  = V90O0(end) - V90(end) / V90(end)
    % Opto 90 = V0O90(end) - V0(end)  / V0(end)

    contrastNo=length(V0);
    % Baseline responses
    v0bl=(columnarResponse(:,:,V0(contrastNo)-nblank).*ROImaskNaN);
    v90bl=(columnarResponse(:,:,V90(contrastNo)-nblank).*ROImaskNaN);
    % Opto responses
    opto0biasing=(columnarResponse(:,:,V90O0(contrastNo)-nblank).*ROImaskNaN);
    opto90biasing=(columnarResponse(:,:,V0O90(contrastNo)-nblank).*ROImaskNaN);

    % remove infs
    opto0comparison  = 100 * (opto0biasing-v90bl)./v90bl;
    opto90comparison = 100 * (opto90biasing-v0bl)./v0bl;
    opto0comparison(isinf(opto0comparison))=NaN;
    opto90comparison(isinf(opto90comparison))=NaN;

    opto0BiasingStrength  = nanmean(opto0comparison, 'all');
    opto90BiasingStrength = nanmean(opto90comparison, 'all');
    %}
    ...
    % --- All visual, incongruent "opto-only" vs visual ---
    axes(hAx(2))
    % Strength of opto 90 vs visual 0
    for i=1:length(V0O90)
        img=columnarResponse(:,:,V0O90(1)-nblank).*ROImask;
        imgRef=columnarResponse(:,:,V0(i)-nblank).*ROImask;
        [~,~,~,intensityPrct]=calculateSimilarity(img,imgRef);
        O90V0xVisuali(i)=intensityPrct;
    end
    ...
    % title and labels
    title({'Opto-only vs all visual response', sprintf('Single gabor (S.F.: 6cpd, Con.: %.0f%%)',max(stimCon))},'FontWeight', 'Normal')
