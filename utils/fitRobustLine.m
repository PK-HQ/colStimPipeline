function fitRobustLine(x, y, linecolor)
    % Function to fit a standard linear line to data after removing outliers
    %
    % Args:
    ...
    if numel(x) < 3
        disp('Not enough points to perform fitting (requires at least 3 points).');
        return;
    end

    % Identify outliers using robust fitting
    [~, stats] = robustfit(x, y);
    inlierIdx = stats.w > 0.75; % Keep points with robust weights > 0.5
    xInliers = x(inlierIdx);
    yInliers = y(inlierIdx);

    % Check if enough inliers remain after outlier removal
    if numel(xInliers) < 3
        disp('Not enough inliers to perform standard fitting (requires at least 3 points).');
        return;
    end
