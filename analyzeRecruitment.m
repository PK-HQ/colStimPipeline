...
for sessionID=1:numel(analysisSessID)
    ...
    if isfile(filenameStructCurrent.Intg) % Load imaging data if it exists 
        ...
        %integrated data
        load(filenameStructCurrent.Intg,'DataTrial')
        % .gaussian file
    ...
    opto0ratio=recruitmentPCAMap(1,:);
    opto90effect=recruitmentPCAMap(2,:);
    % Plot congruent: visual 0 and 90, with congruent opto
    plot(stimCon,opto0ratio,'b-','LineWidth',2,'Marker','v','MarkerSize',10,'MarkerFaceColor','b','MarkerEdgeColor','k','DisplayName','Opto-0 (Vis 0 - 90)'); hold on;
    ...
    stimCon=unique(TS.Header.Conditions.StimCon(find(TS.Header.Conditions.TypeCond==3)));
    % congruent optostim+visual
    opto0ratio=-recruitmentPCAMap(1,:)*100./fullPCAMap(1,:);
    opto90ratio=recruitmentPCAMap(2,:)*100./fullPCAMap(2,:);
    % Plot congruent: visual 0 and 90, with congruent opto
    plot(stimCon,opto0ratio,'b-','LineWidth',2,'Marker','v','MarkerSize',10,'MarkerFaceColor','b','MarkerEdgeColor','k','DisplayName','Opto-0 (Vis 0 - 90)'); hold on;
    plot(stimCon,opto90ratio,'r-','LineWidth',2,'Marker','^','MarkerSize',10,'MarkerFaceColor','r','MarkerEdgeColor','k','DisplayName','Opto-90 (Vis 90 - 0)'); hold on;
    % title and labels
    title({'Signed ratio of recruitment versus full response', sprintf('4Hz flashed grating (SF: 2cpd, 100%%)')},'FontWeight', 'Normal')
    xlim([min(stimCon) max(stimCon)]);ylabel('Ratio of r_{recruitment} / r_{full} (%)');
    line(xlim,[0 0],'color',.3*[1 1 1],'lineWidth',1.5,'LineStyle','--','HandleVisibility','off')
    hLeg=legend('location','southeast','NumColumns',1,'FontSize',14);
    % cosmetic
    upFontSize(14,.01)
...
tbl=array2table([analysisSessID' sessionColumns sessionRatio],...
    'VariableNames',{'EntryID' 'Column-H' 'Column-V', 'Ratio-H', 'Ratio-V'});
save([mainPath 'Chip/Meta/recruitment/recruitmentRatios' num2str(numel(analysisSessID)) '.mat'],...
    'analysisSessID','sessionColumns','sessionRatio','tbl')
