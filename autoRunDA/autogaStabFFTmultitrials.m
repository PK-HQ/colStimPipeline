function Message = autogaStabFFTmultitrials(fnTS,TS,DAVersion)
...
totalStimFrames=100;
iFramesFFT = 14:14+totalStimFrames-1;%20:120%14:140;
PeakFreq = 33; 
lShowFig = 1; 
...
%% Do single trial FFT
% Get stim freq per condition (incl blanks)
PeakFreqConds = 1000 ./ StimPeriod;

%% For each cond, get all trials and do FFT @ correct stimFreqCond for stim and blank conds
% Get mean blank for FFT subtraction
if TS.Header.NumBlankCond
    blankTrialIdx=cell2mat(TS.Header.Index.iValidBLKCond( ...
            1:TS.Header.NumBlankCond));
    % load all blanks into a 512 x 512 x 140 x 20-blank mat
    for bb=1:numel(blankTrialIdx)
        tTrialBlanks(:,:,:,bb) = ...
            blkGetData(fullfile(PathName, ...
                                TS.Trial(TS.Header.Index.iBLKTrial(bb)). ...
                                fnBLK.name), ...
                       TS.Header.Imaging.BLKHeader);
    end
  meanBlanks = mean(tTrialBlanks,4);
end
  % grab 512 x 512 x 140 image for single trial
  
% figure
nTrials=TS.Header.Index.nBLKTrial;
for j = 1:nTrials
  ...
  % get trial info: stim cond and desired peak frequency of stimuli
  trialTS=TS.Trial(TS.Header.Index.iBLKTrial(j));
  stimCond=double(extractfield(trialTS,'CurrCond'));
  isBlank=stimCond <= TS.Header.NumBlankCond;
  if ~isBlank

      iPeakFreq = ...
        round(PeakFreqConds(stimCond)/1000*nFrames*TS.Header.Imaging.BLKHeader.FrameDuration)+1;
      if iPeakFreq==1
      ...
      % FFT for nml and FFT (blank)
      NmlTrialBlank(:,:,j) = mean(meanBlanks(:,:,iFramesNmlTrial),3);
    
      tFFTTrialBlank = ...
        fft(meanBlanks(:,:,iFramesFFT)./ ...
            repmat(NmlTrialBlank(:,:,j), [1,1,nFrames]),[],3);
    
      % Def frames for nml and FFT (stim)
      ...
      if (dodetrend)
        ...
        % for blank
        tFFTTrialBlank = reshape(tFFTTrialBlank,[],numel(iFramesFFT));
        B         = FFTtrend(2:end)' \ tFFTTrialBlank(:,2:end)';
        BTrend    = B' * FFTtrend(2:end);
        tFFTTrialBlank(:,2:end) = tFFTTrialBlank(:,2:end) - BTrend;
        tFFTTrialBlank = reshape(tFFTTrialBlank, size(NmlTrialBlank,1), size(NmlTrialBlank,2), numel(iFramesFFT));
      end
    
      % Take FFT frequency slice out of FFT-ed data, blank subtracted
      tFFTTrialBlankSubt=(tFFTTrial(:,:,iPeakFreq) - tFFTTrialBlank(:,:,iPeakFreq));
      DataTrial(:,:,j) = tFFTTrialBlankSubt *FTT2Amp;
      ...
      if lShowFig
        ...
        tFFTTrialSpectrum = ...
          squeeze(abs(mean(mean(tFFTTrialBlankSubt(iHeight,iWidth,:),1),2)))*FTT2Amp;
  ...
  elseif isBlank
    %blanks dont need to be calculated, not useful
    DataTrial(:,:,j)=nan(512,512);
    NmlTrial(:,:,j)=nan(512,512);
  end
end
