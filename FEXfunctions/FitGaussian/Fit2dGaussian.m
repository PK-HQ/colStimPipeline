...
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load('Y:\Chip\Chip20230803\run0\M28D20230803R0OrientationP2.mat')
load('Y:\Chip\Chip20230803\run0\M28D20230803R0TS.mat')
...
%% ---Parameters---
n = 512; m = 512;         % n x m pixels area/data matrix
A0 = [1,200,100,250,100,0.78];   % Inital (guess) parameters
...
% Sample Parameters:
A_s = [1,200,100,250,100,0.7854]; % The original example

% Build Sample data,
S = FilterFermi2D(RespCond(:,:,1), 0, .4, TS.Header.Imaging.SizePxl);%f(A_s,X);

% Generate Noise in sample data: 
%noise=0;%(15/100)*A_s(1); 
%S=S+noise*(rand(size(x))-0.5);
...
text(n/4 + 36,2*m/3+0,sprintf('Amplitude \t X-Coord \t X-Width \t Y-Coord \t Y-Width \t Angle'),'Color','black');
text(n/4,2*m/3+10,sprintf('Initial: \t %1.2f \t %1.2f \t %1.2f \t %1.2f \t %1.2f \t %1.2f',A_s),'Color','blue');
text(n/4,2*m/3+20,sprintf('Fit:      \t %1.2f \t %1.2f \t %1.2f \t %1.2f \t %1.2f \t %1.2f',A),'Color','red');
