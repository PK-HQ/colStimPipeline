function [betaSorted,muSorted,sigmaSorted,contrastsSorted,percentVerticalSorted]=analyzeBiasingBlock(dsCurrentSess,bitmapData,blockID,hAx,saveFlag,saveType)
...
if isempty(filenameStructCurrent.baselineTS) % all opto and baseline conds in same block
    ...
    % Add title
      ...
      h=title({[dsCurrentSess.date 'R' dsCurrentSess.run],...
         ['\beta: ' num2str(betaSorted(1)*100,'%.0f') '(' num2str((bitmapData.nColumns(2,blockID))) ' @ ' num2str(bitmapData.energy(:,2,blockID),2) 'mW)'],...
         [num2str(betaSorted(2)*100,'%.0f') '                      '],...
         ['    ' num2str(betaSorted(3)*100,'%.0f') '(' num2str((bitmapData.nColumns(1,blockID))) ' @ ' num2str(bitmapData.energy(:,1,blockID),2) 'mW)']},...
    ...
    percentVerticalSorted=[percentVerticalOptostim(:,2) percentVerticalBaseline(:,2) percentVerticalOptostim(:,1)];
      ...
      h=title({[dsCurrentSess.date 'R' dsCurrentSess.run],...
         ['\beta: ' num2str(betaOptostim(2)*100,'%.0f') '(' num2str((bitmapData.nColumns(2,blockID))) ' @ ' num2str(bitmapData.energy(:,2,blockID),2) 'mW)'],...
         [num2str(betaBaseline(2)*100,'%.0f') '                      '],...
         ['    ' num2str(betaOptostim(1)*100,'%.0f') '(' num2str((bitmapData.nColumns(1,blockID))) ' @ ' num2str(bitmapData.energy(:,1,blockID),2) 'mW)']},...
         'FontWeight','normal','FontSize',14,'FontName','Arial','interpreter','tex');
